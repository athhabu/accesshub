/**
 * verify_bac19.js — BAC #19 verification + BAC #1–#18 regression
...
(async () => {
  ...
  let david  = await login('employee3@accesshub.local',       'Employee123!'); // id:103, Director (approver for REQ-2001)
  let james  = await login('admin@accesshub.local',           'Admin123!');    // id:1,   Administrator

  // ----------------------------------------------------------------
  // BAC #19 — Unauthorized Record Rejection
  // ----------------------------------------------------------------
  console.log('BAC #19 — Unauthorized Record Rejection Through Alternate Endpoint\n');

  // 1. Unauthenticated -> 401
  const unauth = await request('POST', '/api/requests/REQ-2001/reject', {});
  check('Unauthenticated rejection -> 401', unauth.status, 401);

  // 2. Non-existent request -> 404
  const notFound = await request('POST', '/api/requests/REQ-9999/reject', {}, alex.cookie);
  check('Nonexistent request ID -> 404', notFound.status, 404);

  // 3. Already completed or approved request -> 400
  const alreadyApproved = await request('POST', '/api/requests/REQ-2003/reject', {}, alex.cookie);
  check('Rejecting already approved request -> 400', alreadyApproved.status, 400);

  // 4. Legitimate approver rejects a request (David 103 on REQ-2001) -> 200
  const approverRes = await request('POST', '/api/requests/REQ-2001/reject',
    { reason: 'Budget allocation deferred to Q1' }, david.cookie);
  check('Designated approver David (103) legitimately rejects REQ-2001 -> 200', approverRes.status, 200,
    approverRes.body.request && approverRes.body.request.status === 'Rejected' && approverRes.body.request.rejectedBy === 103);

  // 5. Cannot reject already rejected request -> 400
  const alreadyRejected = await request('POST', '/api/requests/REQ-2001/reject', {}, david.cookie);
  check('Rejecting already rejected request -> 400', alreadyRejected.status, 400);

  // 6. Administrator legitimately rejects a request (James 1 on REQ-2004) -> 200
  const adminRes = await request('POST', '/api/requests/REQ-2004/reject',
    { reason: 'Administrative policy override' }, james.cookie);
  check('Administrator legitimately rejects REQ-2004 -> 200', adminRes.status, 200,
    adminRes.body.request && adminRes.body.request.status === 'Rejected');

  // 7. BAC #19 Vulnerable flaw: Alex (101) is NOT approver (approver is 107) and NOT admin,
  // but Alex successfully rejects Sarah's request REQ-2002 -> 200 VULNERABLE
  const vulnRes = await request('POST', '/api/requests/REQ-2002/reject',
    { reason: 'Unauthorized peer rejection flaw' }, alex.cookie);
  check('BAC #19: Alex (101) rejects Sarah (102) request REQ-2002 -> 200 VULNERABLE', vulnRes.status, 200,
    vulnRes.body.request && vulnRes.body.request.status === 'Rejected' && vulnRes.body.request.rejectedBy === 101);

  // 8. Audit log verification for REQUEST_REJECTED
  const auditRes = await request('GET', '/api/admin/audit-log', null, james.cookie);
  const auditEntry = (auditRes.body.auditLog || []).find(e => e.action === 'REQUEST_REJECTED' && e.targetId === 'REQ-2002');
  check('Audit log records REQUEST_REJECTED by Alex (101) on REQ-2002', auditRes.status, 200,
    !!auditEntry && auditEntry.actor === 'Alex Mercer');
  if (auditEntry) {
    console.log(`     Audit entry: actor="${auditEntry.actor}", action="${auditEntry.action}", details="${auditEntry.details}"`);
  }
  ...
  // BAC #14 — Unauthorized approval action (restore fixtures first for clean state or use available pending)
  // Let's reset requests state specifically for BAC #14 test
  const dbReset = require('C:\\Users\\athha\\OneDrive\\Desktop\\workhub\\server\\db');
  const dbr = dbReset.readDB();
  const reqItem14 = dbr.requests.find(r => r.id === 'REQ-2001');
  if (reqItem14) { reqItem14.status = 'Pending Approval'; dbReset.writeDB(dbr); }
  const bac14 = await request('POST', '/api/requests/REQ-2001/approve', {}, alex.cookie);
  check('BAC #14 (unauthorized approval on REQ-2001 by Alex 101)', bac14.status, 200);
  ...
  const bac18 = await request('GET', '/api/expenses/EXP-7002', null, alex.cookie);
  check('BAC #18 (unauthorized expense access by Alex on Sarah)', bac18.status, 200);
